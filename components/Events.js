...
export default class Events extends Component {
  static navigationOptions = ({ navigate, navigation }) => ({
   title: "Events",
   headerRight: <Button title="Create a New Event" onPress={()=>{ navigation.navigate('createCamp'); }} />,
 })
  constructor(props) {
    super(props);
...
async _campPress(item) {
    ...
    await AsyncStorage.setItem('event', stringItem);
  ...
  async makeRemoteRequest() {
      ...
      let events = await API.post('CampVue', '/getevents', myInit);
      //camps = camps.json();
      //Alert.alert(camps);
      this.setState({
          data: events.results,
    ...
    return (      
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            <ListItem
              title={`${item.EventDateTime} ${item.EventName}`}
